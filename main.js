document.addEventListener('DOMContentLoaded', function () {
    ...
    function createItemElement(item) {
        ...
        el.dataset.itemId = item.id; // used for event delegation lookup

        return el;
    }

    // find the ITEMS entry that matches a given id
    ...
    // even ones re-created by startGame(). Works for mouse, touch and pen.
    itemsEl.addEventListener('pointerdown', function (event) {
        var el = event.target.closest('.game-item');
        if (!el) {
            return;
        }
        var item = findItemById(el.dataset.itemId);
        if (!item) {
            return;
        }
    ...
    function startDrag(event, el, item) {
        ...
        // pointer capture keeps sending move/up events to this element
        // even if the finger/cursor moves off it while dragging
